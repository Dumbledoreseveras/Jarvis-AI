...
async def TextToAudioFile(text) -> None:
    ...
    if os.path.exists(file_path): # Check if the file already exists
        os.remove(file_path)    # If it exists, remove it to avoid overwrittings errors

    # Create the communicate object to generate speech
    communicate = edge_tts.Communicate(text, AssistantVoice, pitch = '+5Hz', rate = '-13%')
    await communicate.save(r'Data\speech.mp3') # Save the generate speech as an MP3 file 
...
def TTS(Text, func=lambda r=None: True):
    while True:
        try:
            ...
            try:
                # Call the provided func with false to signal the end of TTS
                func(False)
...
if __name__ == "__main__":
    while True:
        # Prompt user for input and pass itto TextToSpeech function
